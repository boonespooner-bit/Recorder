...
This app currently lives in a subfolder of another repo. For a clean deploy story, give it its own repo:

```bash
# from the picture-perfect directory
git init
git add .
git commit -m "Picture Perfect initial app"
gh repo create picture-perfect --private --source=. --push
# (or create an empty repo on github.com and: git remote add origin <url> && git push -u origin main)
```

(You can also deploy straight from this subfolder — Render supports a "Root Directory" setting — see below.)

## 4. Deploy on Render

**Option A — Blueprint (own repo):** with `render.yaml` at the repo root, go to Render → **New + → Blueprint**, pick the repo, and Render creates the service and persistent disk automatically. It will prompt you for `GEMINI_API_KEY`.

**Option B — Manual (subfolder of this repo):**
1. Render → **New + → Web Service** → connect the GitHub repo.
2. **Root Directory:** `picture-perfect`
3. **Build command:** `npm install` · **Start command:** `npm start`
4. **Environment variables:** `GEMINI_API_KEY` = your key, `DATA_DIR` = `/var/data`
5. **Disks:** add a disk mounted at `/var/data` (1 GB is plenty to start). Without a disk, uploads/results vanish on every deploy/restart — Render's filesystem is ephemeral.
6. Deploy. The health check is `/api/health`.
